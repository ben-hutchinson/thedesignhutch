...
import { FounderRevealMotion } from "@/components/motion/founder-reveal-motion";
...
export function AboutSection({
  ...
  const founderIntro = (
    <>
      ...
      <p className="mt-5 max-w-lg text-lg leading-relaxed text-[#c4c4bd] md:ml-11">
        You work directly with me from the first conversation through design,
  ...
  const founderAdditionalCopy = (
    <div className="sr-only">{founderCommitments.slice(3).join(" ")}</div>
  );
  const founderPortrait = founderImage.src ? (
    <Image
